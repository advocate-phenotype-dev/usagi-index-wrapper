...
- **Hybrid tokenization**: the current pure character n-gram approach produces false positives from shared character sequences between unrelated words (e.g. "nipple" matching "Whipple" via `ipp`, `ppl`, `ple`). The original `UsagiAnalyzer.java` used Porter stemming + `WordDelimiterFilter` and was deliberately replaced with n-grams for better cross-vocabulary coverage. A hybrid — word-level stems for candidate retrieval, character n-grams for scoring — would improve precision for common English clinical terms without sacrificing recall on chemical names, abbreviations, and non-English vocabulary entries
